...
const totalAmount = computed(
  () => widgetStore.paymentAmountToCharge || booking.value?.pricing?.grandTotal || 0
)
const isPrepayment = computed(() => widgetStore.selectedPaymentType === 'prepayment')
...
const effectiveCurrency = computed(() => (currencyConversion.value ? 'TRY' : currency.value))
...
const isFormValid = computed(() => {
  ...
  return (
    cardData.value.holder.trim().length >= 3 &&
    ...
    if (currencyConversion.value || !result.installments || result.installments.length <= 1) {
      ...
      installmentOptions.value = result.installments.map(inst => ({
        ...
        monthlyAmount: inst.amount
          ? Math.round((inst.amount / inst.count) * 100) / 100
    ...
    <div class="order-summary">
      ...
      <div v-if="currencyConversion" class="order-summary-row">
        <span class="order-summary-label"
          >{{ t('payment.amount') }} ({{ currencyConversion.originalCurrency }})</span
        >
        <span class="order-summary-value">{{
          formatCurrency(currencyConversion.originalAmount, currencyConversion.originalCurrency)
        }}</span>
      </div>
      <div v-if="currencyConversion" class="order-summary-row">
        <span class="order-summary-label">{{ t('payment.exchangeRate') }}</span>
        <span class="order-summary-value"
          >1 {{ currencyConversion.originalCurrency }} =
          {{ currencyConversion.exchangeRate }} TRY</span
        >
      </div>
      <div v-if="isPrepayment && !currencyConversion" class="order-summary-row">
        <span class="order-summary-label">{{ t('common.total') }}</span>
        <span class="order-summary-value">{{
          formatCurrency(booking?.pricing?.grandTotal || 0, currency)
        }}</span>
      </div>
      <div class="order-summary-row total">
        <span class="order-summary-label">{{
          isPrepayment ? t('payment.payingPrepayment') : t('payment.amountToPay')
        }}</span>
        <span class="order-summary-value price">{{
          formatCurrency(effectiveAmount, effectiveCurrency)
        }}</span>
      </div>
    </div>
          ...
          >
            ...
            <div class="installment-info">
              ...
              <span v-if="option.count > 1" class="installment-monthly">
                {{ formatCurrency(option.monthlyAmount, effectiveCurrency) }} x
                {{ option.count }} ay
              </span>
            </div>
            <div class="installment-total">
              ...
              <span class="installment-amount">{{
                formatCurrency(option.totalAmount, effectiveCurrency)
              }}</span>
            </div>
          </label>
        </div>
    ...
    <div v-if="show3DModal" class="secure-modal-overlay">
      <div class="secure-modal">
        ...
        <div class="secure-modal-footer">
          <button class="btn-text" @click="cancel3DPayment">
            {{ t('payment.secure3D.cancel') }}
          </button>
        </div>
      </div>
    </div>
