...
const contactEmail = computed(
  () => booking.value?.contact?.email || widgetStore.bookingData.contact?.email
)
...
const roomName = computed(
  () =>
    resolveName(selectedRoom.value?.roomType?.name) || resolveName(booking.value?.roomType?.name)
)
...
<template>
  <div class="confirmation-view">
    ...
    <div class="details-card">
      ...
      <div class="details-header">
        <div class="details-hotel">
          <img
            ...
            :src="widgetStore.resolveAssetUrl(hotelInfo.logo?.url || hotelInfo.images[0].url)"
            :alt="hotelInfo?.name"
      ...
      <div class="details-dates">
        <div class="details-date">
          ...
          <span class="details-date-value">{{
            formatDateLong(searchParams.checkIn || booking?.checkIn)
          }}</span>
        </div>
        ...
        <div class="details-date">
          ...
          <span class="details-date-value">{{
            formatDateLong(searchParams.checkOut || booking?.checkOut)
          }}</span>
        </div>
      </div>
        ...
        <div class="details-item">
          ...
          <span>
            ...
            <template v-if="(searchParams.children?.length || booking?.guests?.children) > 0">
              , {{ searchParams.children?.length || booking?.guests?.children }}
              {{ t('common.children') }}
            </template>
          </span>
        </div>
    ...
    <div class="actions">
      ...
      <button
        v-if="widgetStore.config.mode === 'floating'"
