...
export const create = asyncHandler(async (req, res) => {
  ...
  const existingSlug = await SubscriptionService.findOne({ slug: finalSlug }).select('_id').lean()
  if (existingSlug) {
    const regex = new RegExp(`^${finalSlug}(-\\d+)?$`)
    const siblings = await SubscriptionService.find({ slug: regex }).select('slug').lean()
    let max = 1
