...
function getCurrentStage(tx) {
  if (!tx.logs || tx.logs.length === 0)
    return { stage: 'pending', text: 'Bekliyor', icon: 'hourglass_empty' }
...
<template>
  <div>
    ...
    <div v-if="store.stats" class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      <div
        class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border dark:border-slate-700 p-5 hover:shadow-md transition-shadow"
      >
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="w-12 h-12 rounded-lg bg-red-100 dark:bg-red-900/30 flex items-center justify-center"
          >
    ...
    <!-- Filters -->
    <div
      class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border dark:border-slate-700 p-5 mb-6"
    >
    ...
    >
      <table class="w-full">
        <thead class="bg-gray-50 dark:bg-slate-900 border-b dark:border-slate-700">
          <tr>
            <th
              class="px-4 py-4 text-left text-xs font-semibold text-gray-600 dark:text-slate-300 uppercase tracking-wider"
            >
              Date
            </th>
            <th
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
          ...
          >
            ...
            <td class="px-4 py-4">
              <code
                class="text-xs bg-gray-100 dark:bg-slate-700 px-2 py-1 rounded font-mono text-gray-700 dark:text-slate-300"
              >
                {{ getOrderId(tx) }}
              </code>
            </td>
            <td
              class="px-4 py-4 text-sm font-mono text-gray-700 dark:text-slate-300 relative group"
            >
            ...
            <td class="px-4 py-4">
              <span
                :class="[
                  'inline-flex items-center gap-1 px-2.5 py-1 text-xs rounded-full font-medium border',
            ...
            <td class="px-4 py-4 text-right">
              <button
                @click.stop="showTransactionDetail(tx)"
      ...
      <div
        ...
        class="px-4 py-4 border-t dark:border-slate-700 bg-gray-50 dark:bg-slate-900 flex items-center justify-between"
      >
        <div class="text-sm text-gray-600 dark:text-slate-400">
          Showing
          <span class="font-medium">{{ (pagination.page - 1) * pagination.limit + 1 }}</span> to
    ...
    >
      ...
      >
        <!-- Modal Header -->
        <div
          class="px-6 py-4 border-b dark:border-slate-700 bg-gray-50 dark:bg-slate-900 flex justify-between items-center shrink-0"
        >
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <div class="mb-8">
            <div class="flex items-center justify-between relative">
              <div
                class="absolute top-5 left-0 right-0 h-0.5 bg-gray-200 dark:bg-slate-700 -z-10"
              ></div>
              <div v-for="step in progressSteps" :key="step.key" class="flex flex-col items-center">
                <div
                  :class="[
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div class="bg-gray-50 dark:bg-slate-700 rounded-xl p-4">
              <label
                ...
                >Order ID</label
              >
              <div class="font-mono text-sm mt-1 font-medium dark:text-white">
                {{ getOrderId(selectedTransaction) }}
              </div>
            </div>
            <div class="bg-gray-50 dark:bg-slate-700 rounded-xl p-4">
              <label
                ...
                >Amount</label
              >
              <div class="text-lg font-bold mt-1 dark:text-white">
            ...
            <div class="bg-gray-50 dark:bg-slate-700 rounded-xl p-4">
              <label
                ...
                >Installment</label
              >
              <div class="font-semibold mt-1 dark:text-white">
                {{ selectedTransaction.installment || 1 }} Taksit
              </div>
            </div>
            <div class="bg-gray-50 dark:bg-slate-700 rounded-xl p-4">
              <label
            ...
            <div class="bg-gray-50 dark:bg-slate-700 rounded-xl p-4">
              <label
                ...
                >Card</label
              >
              <div class="font-mono mt-1 dark:text-white">
                {{ maskCardNumber(selectedTransaction.card?.maskedNumber) }}
              </div>
            </div>
            <div class="bg-gray-50 dark:bg-slate-700 rounded-xl p-4">
              <label
                ...
                >POS</label
              >
              <div class="font-medium mt-1 dark:text-white">
                {{ selectedTransaction.pos?.name || '-' }}
              </div>
            </div>
            <div class="bg-gray-50 dark:bg-slate-700 rounded-xl p-4">
              <label
                ...
                >Created</label
              >
              <div class="text-sm mt-1 dark:text-white">
                {{ formatDate(selectedTransaction.createdAt) }}
              </div>
            </div>
            <div
              v-if="selectedTransaction.completedAt"
              class="bg-gray-50 dark:bg-slate-700 rounded-xl p-4"
            >
              <label
                class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider"
                >Completed</label
          ...
          <!-- Result -->
          <div
            v-if="selectedTransaction.result"
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                >
                  <span
                    v-if="selectedTransaction.result.code"
                    class="font-mono bg-white/50 dark:bg-slate-800/50 px-1.5 py-0.5 rounded mr-2"
                    >[{{ selectedTransaction.result.code }}]</span
                  >
                  {{ selectedTransaction.result.message }}
                </p>
                <div
                  v-if="selectedTransaction.result.authCode || selectedTransaction.result.refNumber"
                  class="mt-3 flex gap-4 text-sm"
                >
                  <div v-if="selectedTransaction.result.authCode">
                    <span class="text-green-600">Auth Code:</span>
                    <code class="ml-1 font-mono bg-white/50 px-1.5 py-0.5 rounded">{{
                      selectedTransaction.result.authCode
                    }}</code>
                  </div>
                  <div v-if="selectedTransaction.result.refNumber">
                    <span class="text-green-600">Ref Number:</span>
                    <code class="ml-1 font-mono bg-white/50 px-1.5 py-0.5 rounded">{{
                      selectedTransaction.result.refNumber
                    }}</code>
                  </div>
                </div>
              </div>
          ...
          <div v-if="selectedTransaction.logs?.length" class="mt-6">
            ...
            <div class="border-b dark:border-slate-700 mb-4">
              <div class="flex gap-1">
                <button
                  v-for="(log, idx) in selectedTransaction.logs"
                  :key="idx"
            ...
            <!-- Active Log Content -->
            <div
              v-if="selectedTransaction.logs[activeLogTab]"
            ...
            >
                ...
                <div class="flex items-center gap-2">
                  <span
                    :class="[
                      ...
                      getLogTypeBadge(selectedTransaction.logs[activeLogTab].type)
                    ]"
                  >
                    {{ getLogTypeName(selectedTransaction.logs[activeLogTab].type) }}
                  </span>
                  <span class="text-gray-400 text-sm">{{
                    formatLogDate(selectedTransaction.logs[activeLogTab].at)
                  }}</span>
                </div>
              </div>
              ...
              <div class="grid md:grid-cols-2 divide-x divide-gray-700">
                ...
                <div class="p-4">
                  <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-blue-400 font-medium text-sm uppercase tracking-wider"
                        >Request</span
                      >
                    </div>
                    <button
                      @click="copyToClipboard(selectedTransaction.logs[activeLogTab].request)"
                  ...
                  <pre
                    class="text-sm font-mono leading-relaxed overflow-x-auto max-h-96 text-gray-300"
                    v-html="syntaxHighlight(selectedTransaction.logs[activeLogTab].request)"
                  ></pre>
                </div>
                ...
                <div class="p-4">
                  <div class="flex items-center justify-between mb-3">
                    ...
                    <button
                      @click="copyToClipboard(selectedTransaction.logs[activeLogTab].response)"
                      class="p-1.5 rounded hover:bg-gray-700 text-gray-400 hover:text-white transition-colors"
                      title="Kopyala"
                    >
                      <span class="material-icons text-sm">content_copy</span>
                  ...
                  <pre
                    class="text-sm font-mono leading-relaxed overflow-x-auto max-h-96 text-gray-300"
                    v-html="syntaxHighlight(selectedTransaction.logs[activeLogTab].response)"
                  ></pre>
                </div>
              </div>
              ...
              <div
                v-if="
                  selectedTransaction.logs[activeLogTab].type === '3d_callback' &&
                  selectedTransaction.secure?.decrypted
                "
              ...
              >
                <div class="flex items-center justify-between mb-3">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-amber-400 font-medium text-sm uppercase tracking-wider"
                      >Decrypted Data</span
                    >
                  </div>
                  <button
                    @click="copyToClipboard(selectedTransaction.secure.decrypted)"
                    class="p-1.5 rounded hover:bg-gray-700 text-gray-400 hover:text-white transition-colors"
                    title="Kopyala"
                  >
                    <span class="material-icons text-sm">content_copy</span>
                ...
                <pre
                  class="text-sm font-mono leading-relaxed overflow-x-auto max-h-96 text-gray-300"
                  v-html="syntaxHighlight(selectedTransaction.secure.decrypted)"
                ></pre>
              </div>
            </div>
          </div>
          ...
          <div v-if="selectedTransaction.childTransactions?.length" class="mt-8">
            ...
            <div class="space-y-4">
              <div
                v-for="child in selectedTransaction.childTransactions"
                :key="child._id"
                class="border dark:border-slate-700 rounded-xl overflow-hidden"
              >
                <!-- Child Header -->
                <div
                  class="bg-gray-50 dark:bg-slate-700 px-4 py-3 flex items-center justify-between border-b dark:border-slate-600"
                >
                  <div class="flex items-center gap-3">
                    <div
                  ...
                  <div class="flex items-center gap-3">
                    <span class="font-semibold dark:text-white">{{
                      formatAmount(child.amount, selectedTransaction.currency)
                    }}</span>
                    <span
                      :class="[
                ...
                >
                  <div class="flex items-center gap-2">
                    ...
                    >
                      <span
                        v-if="child.result.code"
                        class="font-mono text-xs bg-white/50 dark:bg-slate-800/50 px-1.5 py-0.5 rounded mr-2"
                        >[{{ child.result.code }}]</span
                      >
                ...
                <div v-if="child.logs?.length" class="border-t dark:border-slate-700">
                  <details class="group">
                    <summary
                      class="px-4 py-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center justify-between"
                    >
                      <span
                        class="text-sm font-medium text-gray-600 dark:text-slate-300 flex items-center gap-2"
                      >
                        <span class="material-icons text-sm text-gray-400 dark:text-slate-500"
                          >article</span
                        >
                        {{ child.logs.length }} Log Kaydı
                      </span>
                      <span
                        class="material-icons text-gray-400 dark:text-slate-500 group-open:rotate-180 transition-transform"
                        >expand_more</span
                      >
                    </summary>
                      ...
                      <div v-for="(log, idx) in child.logs" :key="idx" class="mb-4 last:mb-0">
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <pre
                              class="text-xs font-mono text-gray-300 overflow-x-auto max-h-40"
                              v-html="syntaxHighlight(log.request)"
                            ></pre>
                          </div>
                          <div>
                            ...
                            <pre
                              class="text-xs font-mono text-gray-300 overflow-x-auto max-h-40"
                              v-html="syntaxHighlight(log.response)"
                            ></pre>
                          </div>
                        </div>
                      </div>
