...
const currentPartnerId = computed(() => {
  ...
  if (authStore.user?.accountType === 'partner') {
...
const formattedCardNumber = computed({
  ...
  set: val => {
    form.value.cardNumber = val.replace(/\s/g, '')
  }
})
...
const cardType = computed(() => {
  ...
  if (/^5[1-5]/.test(num) || /^2[2-7]/.test(num))
    return { type: 'mastercard', color: 'bg-red-600', textColor: 'text-red-400' }
  if (/^3[47]/.test(num))
    return { type: 'amex', color: 'bg-green-600', textColor: 'text-green-400' }
...
async function handleSubmit() {
  ...
  try {
    ...
    // Partner context: partnerId ekle
    ...
    } else if (currentPartnerId.value) {
      paymentData.partnerId = currentPartnerId.value
    }
  ...
  <div class="max-w-6xl mx-auto">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 mb-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wide">
          Test Kartlari
        </h3>
      ...
      <p
        v-if="testCardType === 'mock'"
        class="text-sm text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 rounded-lg p-3 mb-4 flex items-center gap-2"
      >
        <span class="material-icons text-lg">science</span>
        Mock kartlar gercek banka baglantisi olmadan test icin kullanilir. Test POS sectiginizden
        emin olun.
      </p>
        ...
        >
          ...
          <div class="relative">
            <div class="flex items-center justify-between mb-2">
              ...
              <img
                v-if="getBrandIcon(card.brand)"
                :src="getBrandIcon(card.brand)"
                class="h-6 w-auto"
                :alt="card.brand"
              />
            </div>
            ...
            <div v-if="card.description" class="text-xs text-gray-400 dark:text-slate-500 mt-1">
              {{ card.description }}
            </div>
          </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
        <form @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <div>
            ...
            <p
              v-if="form.paymentType === 'subscription'"
          ...
          <div>
            ...
            >
              ...
              <option v-for="pos in activePosList" :key="pos._id" :value="pos._id">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <div class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                <span v-if="binLoading" class="material-icons animate-spin text-gray-400"
                  >refresh</span
                >
                <span
                  v-else-if="cardType"
                  :class="[
                    'px-2 py-1 text-white text-xs rounded uppercase font-bold',
                    cardType.color
                  ]"
                >
                  {{ cardType.type }}
          ...
          <div
            v-if="(form.currency === 'TRY' || conversionInfo) && binInfo?.installments?.length > 1"
          >
            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2"
              >Installment</label
            >
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg p-3 space-y-1">
                  <div class="flex justify-between text-sm">
                    ...
                    <span class="font-medium text-gray-800 dark:text-white"
                      ...
                      {{ conversionInfo.originalCurrency }}</span
                    >
                  </div>
                  <div class="flex justify-between text-sm">
                    <span class="text-gray-600 dark:text-slate-400">Kur (TCMB):</span>
                    <span class="font-mono text-gray-800 dark:text-white"
                      >1 {{ conversionInfo.originalCurrency }} =
                      {{ conversionInfo.rate?.toFixed(4) }} TRY</span
                    >
                  </div>
                  <div
                    class="flex justify-between text-sm pt-2 border-t border-gray-200 dark:border-slate-600"
                  >
                    ...
                    <span class="font-bold text-lg text-green-600 dark:text-green-400"
                      >{{ conversionInfo.convertedAmount?.toFixed(2) }} TRY</span
                    >
                  </div>
                </div>
              </div>
          ...
          <div
            v-if="error"
            class="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-700 dark:text-red-400 flex items-start gap-3"
          >
          ...
          >
            ...
            <span v-else-if="conversionInfo"
              >Pay {{ conversionInfo.convertedAmount?.toFixed(2) }} TRY ({{ form.amount }}
              {{ form.currency }})</span
            >
      ...
      <div class="space-y-6">
        ...
        <div
          :class="[
            ...
            cardType
              ? cardType.color.replace('bg-', 'from-') + ' to-gray-900'
          ...
          <div class="space-y-3">
            ...
            >
              ...
              <span class="font-medium dark:text-white flex items-center gap-1">
                <span
                  v-if="binInfo.bin?.country === 'TR' || binInfo.country === 'TR'"
                  class="text-xs px-2 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded"
                  >TR</span
                >
            ...
            <div class="flex justify-between items-center py-2">
              ...
              <span class="font-medium text-primary-600 dark:text-primary-400">{{
                binInfo.pos?.name || 'Auto-selected'
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div
          v-if="result"
          class="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 border-2"
          :class="
            result.success
        ...
        >
          ...
          >
            ...
            <div>
              <span class="font-semibold text-xl block">{{
          ...
          <div class="space-y-2 text-sm bg-gray-50 dark:bg-slate-700 rounded-lg p-4">
            ...
            <div v-if="result.amount" class="flex justify-between">
              ...
              <span class="font-medium dark:text-white"
                >{{ result.amount }} {{ result.currency }}</span
              >
            </div>
          ...
          <button
            @click="resetForm"
            class="mt-4 w-full py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 font-medium transition-colors"
          >
            New Payment
          </button>
        </div>
        ...
        <details v-if="binInfo || result" class="bg-gray-100 dark:bg-slate-800 rounded-xl">
          <summary
            class="p-4 cursor-pointer text-sm text-gray-600 dark:text-slate-400 hover:text-gray-800 dark:hover:text-white"
          >
            <span class="ml-2">Debug: API Response</span>
          </summary>
          <pre
            class="p-4 text-xs overflow-auto max-h-60 bg-gray-800 dark:bg-slate-900 text-green-400 rounded-b-xl"
            >{{ JSON.stringify(binInfo || result, null, 2) }}</pre
          >
        </details>
      </div>
    </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg">
        <div class="p-4 border-b dark:border-slate-700 flex justify-between items-center">
          ...
          <button
            ...
            class="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg transition-colors dark:text-slate-300"
          >
