<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-slate-700">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-slate-400">{{ $t('emailLogs.totalEmails') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-slate-700">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.successRate || 0 }}%</p>
            <p class="text-sm text-gray-500 dark:text-slate-400">{{ $t('emailLogs.successRate') }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="flex flex-wrap gap-4">
        ...
        <div class="flex-1 min-w-[150px]">
          ...
          >
            ...
            <option value="booking-confirmation">{{ $t('emailLogs.typeBookingConfirmation') }}</option>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
      ...
      <div v-else-if="logs.length === 0" class="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-slate-400">
      ...
      <table v-else class="w-full">
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
          ...
          >
            ...
            <td class="px-4 py-3">
              <div class="text-sm text-gray-900 dark:text-white max-w-xs truncate">{{ log.subject }}</div>
            </td>
            <td class="px-4 py-3">
              <span
            ...
            <td class="px-4 py-3">
              ...
              >
                <span class="w-1.5 h-1.5 rounded-full mr-1.5" :class="getStatusDotClass(log.status)"></span>
            ...
            <td class="px-4 py-3">
              <div v-if="log.error?.message" class="text-sm text-red-600 dark:text-red-400 max-w-xs truncate" :title="log.error.message">
                {{ log.error.message }}
              </div>
      ...
      <!-- Pagination -->
      <div v-if="pagination.pages > 1" class="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-slate-700">
...
const getTypeClass = (type) => {
  const classes = {
    ...
    'booking-confirmation': 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300',
