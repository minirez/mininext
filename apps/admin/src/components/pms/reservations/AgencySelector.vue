    ...
    <div class="relative">
      ...
      >
        ...
        <span v-else class="flex-1 text-gray-400 dark:text-slate-500">
          {{ placeholder || $t('reservation.selectAgency') }}
        </span>
          ...
          <form class="p-6 space-y-4" @submit.prevent="createAgency">
            <div>
              ...
              <input
                ...
                :placeholder="$t('reservation.companyNamePlaceholder')"
              />
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                {{ $t('reservation.tradeName') }}
              </label>
              <input
                v-model="newAgency.tradeName"
                type="text"
                class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                :placeholder="$t('reservation.tradeNamePlaceholder')"
              />
            </div>
...
const createAgency = async () => {
  ...
  try {
    ...
    toast.success(t('reservation.agencyCreated'))
  } catch (err) {
