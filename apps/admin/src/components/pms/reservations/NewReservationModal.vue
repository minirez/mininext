        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <CountrySelect
              ...
              :placeholder="$t('reservation.selectNationality')"
            />
          </div>
        </div>
      </div>
...
const errors = computed(() => {
  ...
  if (!form.value.checkIn) errs.checkIn = t('reservation.errors.checkInRequired')
  if (!form.value.checkOut) errs.checkOut = t('reservation.errors.checkOutRequired')
  if (nights.value <= 0) errs.checkOut = t('reservation.errors.invalidDates')
  if (!form.value.roomTypeId) errs.roomTypeId = t('reservation.errors.roomTypeRequired')
  if (!form.value.mealPlanId) errs.mealPlanId = t('reservation.errors.mealPlanRequired')
  if (!form.value.leadGuest.firstName?.trim())
    errs.firstName = t('reservation.errors.firstNameRequired')
  if (!form.value.leadGuest.lastName?.trim())
    errs.lastName = t('reservation.errors.lastNameRequired')
  ...
  if (form.value.contact.email?.trim() && !isValidEmail(form.value.contact.email)) {
    errs.email = t('reservation.errors.emailInvalid')
  }
  if (!form.value.pricing.total || form.value.pricing.total <= 0)
    errs.total = t('reservation.errors.totalRequired')
